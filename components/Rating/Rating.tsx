...
export const Rating = forwardRef(({rating, setRating, isEdit = false, ...props}: RatingProps, ref: ForwardedRef<HTMLDivElement>): JSX.Element => {
    const [ratingArray, setRatingArray] = useState<JSX.Element[]>(new Array(5).fill(<></>));

    const changeDispay = useCallback((ratingIndex: number) => {
        if (!isEdit) {
            return;
        }
    ...
    const onClick = useCallback((ratingIndex: number) => {
        if (!isEdit || !setRating) {
            return;
        }
    ...
    const constructRating = useCallback((rating: number) => {
